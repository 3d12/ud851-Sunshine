...
//  DONE (20) Create a constant int to identify our loader used in DetailActivity
    ...
    private static final int COL_ID_REFERENCE = 0;
    private static final int COL_DATE_REFERENCE = 1;
    private static final int COL_WEATHER_ID_REFERENCE = 2;
    private static final int COL_MIN_TEMP_REFERENCE = 3;
    private static final int COL_MAX_TEMP_REFERENCE = 4;
    private static final int COL_PRESSURE_REFERENCE = 5;
    private static final int COL_HUMIDITY_REFERENCE = 6;
    private static final int COL_WIND_SPEED_REFERENCE = 7;
    private static final int COL_WIND_DIRECTION_REFERENCE = 8;
...
//      DONE (13) Find each of the TextViews by ID
        ...
        mDescription = (TextView) this.findViewById(R.id.tv_description);
        mHighTemp = (TextView) this.findViewById(R.id.tv_high_temp);
        mLowTemp = (TextView) this.findViewById(R.id.tv_low_temp);
        mHumidity = (TextView) this.findViewById(R.id.tv_humidity);
        mPressure = (TextView) this.findViewById(R.id.tv_pressure);
        mWindSpeed = (TextView) this.findViewById(R.id.tv_wind_speed);
        mWindDirection = (TextView) this.findViewById(R.id.tv_wind_direction);
...
//      DONE (35) Initialize the loader for DetailActivity
    ...
    public void onLoaderReset(Loader<Cursor> loader) {
        // blank method per instructions
    }

    @Override
    public Loader<Cursor> onCreateLoader(int id, Bundle args) {
        if (id == ID_DETAIL_LOADER) {
            Log.d("DetailActivity", "current mUri is " + mUri.toString());
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        ...
        if (data.getCount() <= 0) {
            Log.d("DetailActivity", "The returned cursor was empty (size <= 0)");
            return;
        }
        data.moveToFirst();
        mDate.setText(SunshineDateUtils.getFriendlyDateString(getBaseContext(),
                data.getLong(data.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_DATE)),
                true));
        mDescription.setText(SunshineWeatherUtils.getStringForWeatherCondition(getBaseContext(),
                data.getInt(data.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_WEATHER_ID))));
        mHighTemp.setText(SunshineWeatherUtils.formatTemperature(getBaseContext(),
                data.getDouble(data.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_MAX_TEMP))));
        mLowTemp.setText(SunshineWeatherUtils.formatTemperature(getBaseContext(),
                data.getDouble(data.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_MIN_TEMP))));
        mHumidity.setText(String.valueOf(data.getDouble(data.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_HUMIDITY))));
        mWindSpeed.setText(String.valueOf(data.getDouble(data.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_WIND_SPEED))));
        mWindDirection.setText(String.valueOf(data.getDouble(data.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_DEGREES))));
        mPressure.setText(String.valueOf(data.getDouble(data.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_PRESSURE))));
        mForecastSummary = mDate.getText().toString() + " " +
                mDescription.getText().toString() + " " +
                mHighTemp.getText().toString() + " " +
                mLowTemp.getText().toString() + " " +
                mHumidity.getText().toString() + " " +
                mWindSpeed.getText().toString() + " " +
                mWindDirection.getText().toString() + " " +
